    ...
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        if (hd->hd_calls[i]) {
            ...
            if ((hd->hd_calls[i]->method == method)){          // First match methods
                if (hd->config.uri_match_fn == NULL){
                    if((strcmp(hd->hd_calls[i]->uri, uri) == 0)) {     // Then match uri strings

                        return i;
                    }
                }else if(hd->config.uri_match_fn != NULL && hd->config.uri_match_fn(
                    hd->hd_calls[i]->uri,
                    uri,
                    strlen(hd->hd_calls[i]->uri)
                ))
                {
                    return i;
                }
    ...
    *err = 0;
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        if (hd->hd_calls[i]) {
            ...
            }else if(hd->config.uri_match_fn != NULL && hd->config.uri_match_fn(hd->hd_calls[i]->uri, uri, strlen(hd->hd_calls[i]->uri)))
            {
                if (hd->hd_calls[i]->method == method)  {               // Finally match methods
                    return hd->hd_calls[i];
                }
...
bool httpd_uri_match_wildcard(const char *reference_uri, const char *uri_to_match, size_t match_upto)
{
    ...
    {
        bool result=true;
        for (size_t i=0; i<match_upto;i++)
        {
            if (reference_uri[i] == '*') {break;}
            else if(reference_uri[i] != uri_to_match[i]) {result=false;break;}
